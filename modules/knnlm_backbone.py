...
class KNNWrapper(object):
    def __init__(
        ...
        self.dist_func = dist_type_to_dist_func[knn_sim_func]
    ...
    def knns_to_log_prob(self, knns, neg_dists):
        ...
        knn_log_probs = (
            ...
            .to(self.device)
            .scatter_add(dim=-1, index=vals_at_knns.long().to(self.device), src=probs)
            .log()
...
class KNNSaver(object):
    ...
    def register_hook(self, layer, func, pre=False):
        #! func should be defined as f(module, input, output)
        handle = (
            layer.register_forward_pre_hook(func)
            if pre
            else layer.register_forward_hook(func)
